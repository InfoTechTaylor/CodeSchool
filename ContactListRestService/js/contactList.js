$(document).ready(function () {
     ...
     $('#add-button').click(function (event) {
          ...
          $.ajax({
               ...
               'dataType': 'json',
               success: function() {
                    // empty any errors and empty the add contact form
                    $('#errorMessages').empty();
                    $('#add-first-name').val('');
                    $('#add-last-name').val('');
                    $('#add-company').val('');
                    $('#add-phone').val('');
                    $('#add-email').val('');
                    // reload contacts so the new contact is in the view
                    loadContacts();
               },
               error: function() {
                    $('#errorMessages')
...
function checkAndDisplayValidationErrors(input) {
     ...
     if (errorMessages.length > 0) {
          ...
          //return true, indicating that there were errorMessage
          return true;
     } else {
