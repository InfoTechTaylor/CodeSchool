...
public class CreateListLocationWebServiceImplTest {
    ...
    @Test
    public void deleteLocationCmdModel() throws Exception{
        // arrange
        Location location = testHelperMethodsObj.createTestLocations(1).get(0);
        testHelperMethodsObj.createTestOrgs(1, location).get(0);
        testHelperMethodsObj.createTestSightings(1, location).get(0);

        Location location2 = testHelperMethodsObj.createTestLocations(1).get(0);

        // act & assert doesn't delete when there are relationships
        try {
            createListLocationWebService.deleteLocationCmdModel(location.getId());
            fail("Expected RelationshipFoundException never thrown");
        } catch (RelationshipFoundException e) {
            // do nothing, test passes
        }

        // act & assert, assert delete works on location with no relationship
        createListLocationWebService.deleteLocationCmdModel(location2.getId());
        Location locationFromDB = locationService.read(location2);

        assert locationFromDB == null;
    }
}