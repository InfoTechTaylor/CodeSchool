...
public class ContactListDaoDbImpl implements ContactListDao {
    ...
    private static final String SQL_INSERT_CONTACT
            ...
            + "values (?, ?, ?, ?, ?)";
    private static final String SQL_DELECT_CONTACT
    ...
    public void removeContact(long contactId) {
        jdbcTemplate.update(SQL_DELECT_CONTACT, contactId);
    }
    ...
    public Contact getContactById(long contactId) {
        try {
            ...
            // there were no results for teh given contact id - we just want to return null in this case
            return null;
        }
    }
    ...
    public List<Contact> searchContacts(Map<SearchTerm, String> criteria) {
        if (criteria.isEmpty()){
            ...
            int paramPosition = 0;
            // we'll put the positional parameters into an array, the order of the parameters will
            // match the order in which we get the search criteria from the map
            String[] paramVals = new String[numParams];
            Set<SearchTerm> keySet = criteria.keySet();
            Iterator<SearchTerm> iter = keySet.iterator();
            // build up the where clause based on the key/value pairs in the
            // map build where clause and positional parameter array
            while (iter.hasNext()) {
                SearchTerm currentKey = iter.next();
                // if we are not the first one in, we must add an AND to the where clause
                if (paramPosition > 0 ) {
                    sQuery.append(" and ");
                }
                ...
                sQuery.append(" = ? ");
                // grab the value for this search criteria and put it into the paramVals array
